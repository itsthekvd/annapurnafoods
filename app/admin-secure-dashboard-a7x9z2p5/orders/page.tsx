...
export default function AdminOrdersPage() {
  ...
  // Render mobile order card - IMPROVED FOR BETTER UX
      ...
      <Card key={order.id} className="mb-4">
        <CardContent className="p-4">
          ...
          <div className="flex justify-between items-start mb-3">
            <div>
              ...
              <div className="flex items-center mt-1">
                <a href={`tel:${order.customerInfo.phone}`} className="flex items-center text-blue-600 text-sm">
          ...
          <div className="bg-gray-50 p-3 rounded-lg mb-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <p className="font-medium text-sm">{order.mealType || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Plan:</p>
                <p className="font-medium text-sm">{order.planType || "One-time"}</p>
              </div>
              <div>
            ...
            {order.deliveryDate && (
              <div className="mt-2 flex items-center text-sm">
                ...
                <span className="text-xs">Delivery: {new Date(order.deliveryDate).toLocaleDateString()}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2">
            ...
            {order.customerInfo.phone && (
              <Button className="flex-1 h-12 text-base bg-green-600 hover:bg-green-700" asChild>
                <a
  ...
  // Order details dialog for mobile
        ...
        <DialogContent className="max-w-full w-[95vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="bg-amber-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                <div>
                  ...
                  <a href={`tel:${selectedOrder.customerInfo.phone}`} className="font-medium text-blue-600">
                ...
                {selectedOrder.customerInfo.zipcode && (
                  <div>
                    ...
                    <p className="font-medium">{selectedOrder.customerInfo.zipcode}</p>
                  </div>
                )}
                {selectedOrder.customerInfo.hasVoiceNote && (
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 gap-2">
                <div>
                  ...
                  <p className="font-medium">{selectedOrder.locationInfo.address}</p>
                </div>
                {selectedOrder.locationInfo.mapUrl && (
                  <div>
                    <a
                      href={selectedOrder.locationInfo.mapUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 flex items-center"
                    >
                      <Eye className="h-4 w-4 mr-1" /> View on Google Maps
                    </a>
                  </div>
                )}
                {selectedOrder.deliveryDate && (
                  <div>
                    ...
                    <p className="font-medium">{new Date(selectedOrder.deliveryDate).toLocaleString()}</p>
                  </div>
                )}
                {selectedOrder.deliveryFrequency && (
                  <div>
                    <p className="text-sm text-gray-500">Frequency:</p>
                    <p className="font-medium">{selectedOrder.deliveryFrequency}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                  ...
                  <div key={index} className="border-b pb-2">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">
                        {item.subscriptionOption && item.subscriptionOption !== "one-time"
                          ? `${item.subscriptionOption} (${item.subscriptionDays} days)`
                          : "One-time"}
                      </span>
                ...
                {selectedOrder.couponCode && (
                  <div className="text-green-600 text-sm">Coupon applied: {selectedOrder.couponCode}</div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                {selectedOrder.paymentInfo.transactionId && (
                  <div>
                    ...
                    <p className="font-medium font-mono text-xs">{selectedOrder.paymentInfo.transactionId}</p>
                  </div>
                )}
                <div>
                  <p className="text-sm text-gray-500">Amount:</p>
                  <p className="font-medium">₹{selectedOrder.paymentInfo.amount.toFixed(2)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              <Button className="flex-1 h-12 text-base" onClick={() => setShowOrderDetails(false)}>
                Close
              </Button>
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          <Link href="/admin-secure-dashboard-a7x9z2p5" className="mr-2">
        ...
        <>
            ...
            <CardContent className="p-4">
              <div className="relative w-full mb-3">
                ...
                <Input
                  ...
                  className="pl-8 w-full h-12 text-base"
                />
              </div>
              ...
              <div className="flex justify-between gap-2">
                ...
                <Button variant="outline" size="lg" className="flex-1 h-12 text-base" onClick={loadOrders}>
              ...
              {showFilters && (
                <div className="mt-3 p-3 bg-gray-50 rounded-md">
                  <h3 className="font-medium text-base mb-2">Filter by Status</h3>
          ...
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="mb-4">
            <TabsList className="grid grid-cols-4 w-full h-14">
              <TabsTrigger value="all" className="text-base py-4">
                All
              </TabsTrigger>
              <TabsTrigger value="pending" className="text-base py-4">
                Pending
              </TabsTrigger>
              <TabsTrigger value="confirmed" className="text-base py-4">
                Processing
              </TabsTrigger>
              <TabsTrigger value="delivered" className="text-base py-4">
                Delivered
              </TabsTrigger>
            </TabsList>
          </Tabs>
