...
interface CartItemProps {
  ...
  getSubscriptionName: (optionId: string) => string
}

const CartItemComponent = ({ item, appliedCoupon, updateQuantity, removeItem, getSubscriptionName }: CartItemProps) => {
  return (
    <Card key={`${item.product.id}-${item.subscriptionOption || "single"}`} className="overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="p-4 flex-1">
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              {item.subscriptionOption && <div className="text-sm text-gray-600">{item.subscriptionDays} day(s)</div>}
              <Button
                variant="ghost"
                size="icon"
...
export default function CartClientPage() {
  ...
  const getSubscriptionName = useCallback((optionId: string) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="space-y-4">
            {cartItems.map((item) => (
              <CartItemComponent
                key={`${item.product.id}-${item.subscriptionOption || "single"}`}
                item={item}
